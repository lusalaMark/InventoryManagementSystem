...
namespace InventoryManagementSystem.CoreBusiness.Validations
{
            ...
            if (product != null)
            {
                if (!ValidatePricing(product))
                    return new ValidationResult(
                        $"The product's price of  {TotalInventoriesCost(product).ToString("c")}!  is less than the inventories cost.",
